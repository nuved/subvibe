...
| B10 | TTML parser reads only `begin`/`end` — `begin`+`dur` timing yields `endMs=0` rows the near-match cache layer can never reach | Narrows cache-heal coverage on file-based sites (ZDF path suspected, unverified live) | A real `dur`-style payload to test against |
| B11 | `fetchSubsByUrl` tags the fetched file with `currentClipId()` at resolve time, not request time | A clip switch mid-fetch could tag the response to the wrong clip (pre-existing race) | Repro; low probability |
| B12 | `liveVideoEl` should ask the adapter FIRST (YT: `.html5-main-video` is deterministic) before the largest-playing heuristic | The heuristic+sticky machinery redoes what adapters know precisely; this bug class keeps recurring at the picker | Small refactor; after the sticky fix soaks |
| B13 | Scrape-mode tick uses raw `video.currentTime` for stamping AND display — degenerate (~0) on relay-clock MSE players | Internally consistent today, so a display-only `playheadMs()` swap would BREAK the invariant; both sides must move together | A real site that scrapes on an MSE player; cuelist covers ZDF/DW |
| B14 | Sync dock is deliberately inert on VOD ("recorded titles are exact") but the UI never says so | A user nudging on YouTube VOD sees a dead control — same "is it broken?" read the live-edge clamp had before its status note | Copy decision: a one-line hint in the timing fold |
| B15 | Line-picker calibration: popup shows the ~4 cues around the playhead, user taps the line matching the audio, offset derived from `cue.startMs − playhead` (all plumbing exists: GET_CLIP-style message + `saveClipSettings`) | Seconds-guessing is the wrong unit — line steps always produce a VISIBLE change, and "no newer line" greys out honestly at the live edge | Own branch after this plan's eye test; keep the fine ±0.25 nudges alongside |
