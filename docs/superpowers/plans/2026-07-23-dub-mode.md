...
- [ ] **Step 1: extend voices tests** (write first, watch fail):
...
First hint in `ttsInstructions` becomes: `"Dub a film line naturally, matching the writing's emotion. Speak at a natural, unhurried pace with clear diction."` Export `isNonSpeechCaption` in `SV_VOICES`. Tests green.
