...
const EXPLAIN_SCHEMA = { name: "sentence_explain", strict: true, schema: { type: "object", additionalProperties: false,
  properties: {
    tr: { type: "string" }, simple: { type: "string" }, g: { type: "string" }, scene: { type: "string" }, who: { type: "array", items: { type: "string" } }, spk: { type: "array", items: { type: "string" } },
  ...
  }, required: ["tr", "simple", "g", "scene", "who", "spk", "words"] } };
...
async function ensureDossier(base, meta, sample, lang) {
  ...
  const run = (async () => {
    ...
    if (d) {
      ...
      // Artwork is free to take late: the wide key art and the episode's stills (no model call, no prefix change).
      if (!d.backdrop && m.backdrop) { d.backdrop = String(m.backdrop).slice(0, 400); dirty = true; }
      if (!(d.stills || []).length && Array.isArray(m.stills) && m.stills.length) { d.stills = m.stills.map((x) => String(x).slice(0, 400)).slice(0, 4); dirty = true; }
      ...
      if (thin && rich) {
        ...
        if (m.backdrop) d.backdrop = String(m.backdrop).slice(0, 400); if (Array.isArray(m.stills) && m.stills.length) d.stills = m.stills.map((x) => String(x).slice(0, 400)).slice(0, 4);
        const was = { kind: d.kind, about: d.about, register: d.register, speakers: d.speakers };
    ...
    d = { v: 1, at: Date.now(), site: String(m.site || ""), title: String(m.title || "").slice(0, 160), show: String(m.show || "").slice(0, 120), season: +m.season || 0, episode: +m.episode || 0,
      ...
      people: t.people, tmdb: t.tmdb, poster: t.poster || String(m.poster || "").slice(0, 400), backdrop: String(m.backdrop || "").slice(0, 400), stills: (Array.isArray(m.stills) ? m.stills : []).map((x) => String(x).slice(0, 400)).slice(0, 4), partial: !!m.partial, sample: lines, tmdbAt: t.tmdb ? Date.now() : 0 };
...
async function explainLine(base, sent, langHint, opts) {
  ...
  const out = { tr: String(p.tr || "").trim(), simple: String(p.simple || "").trim(), g: String(p.g || "").trim(), scene: String(p.scene || "").trim().slice(0, 240), who: Array.isArray(p.who) ? p.who.map((x) => String(x).trim().slice(0, 60)).filter(Boolean).slice(0, 4) : [], spk: Array.isArray(p.spk) ? p.spk.slice(0, Math.max(1, lines.length || 8)).map((x) => String(x || "").trim().slice(0, 60)) : [], lang,
  ...
  return { ok: true, tr: faS(out.tr), simple: out.simple, g: faS(out.g), scene: faS(out.scene), who: out.who, spk: out.spk, lang, explain: explainPref, ctx: cx.ctx || ctx || null, words: out.words.map((x) => ({ w: x.w, m: faS(x.m), pos: x.pos, level: x.level, forms: x.forms, parts: x.parts, register: x.register, tone: x.tone, care: faS(x.care) })) };
}
...
async function faceLookup(name, d) {
  ...
  // Fandom's intro extract is often empty (the page opens with an infobox), so the page's categories carry the check: "Characters in GTA VI", "Grand Theft Auto VI characters" …
  const url = "https://" + d.wiki + ".fandom.com/api.php?action=query&generator=search&gsrsearch=" + encodeURIComponent(name + " " + (d.tag || show)) + "&gsrlimit=4&prop=pageimages%7Cextracts%7Ccategories&piprop=thumbnail&pithumbsize=240&exintro=1&explaintext=1&exchars=300&cllimit=30&format=json&origin=*";
  ...
  for (const p of pages) {
    ...
    if (!t.startsWith(n) || /\/infobox$/.test(t) || /^(characters|list of|category)/.test(t) || !(p.thumbnail && p.thumbnail.source)) continue;
...
function recapPrompt(source, dossier) {
  return `You catch a viewer up on the video they are watching, in ${langName(source)}, with simple words (B1). The user message carries {"scenes":[…one line per passage so far, in order…],"lines":[…the latest subtitle lines…],"upTo":"m:ss"}.\n` +
    ...
    `Return STRICT JSON {"recap":"…","who":["…"],"cast":[{"name":"…","role":"…","weight":"…","note":"…"}]}: recap = what has happened so far, up to this point ONLY — never guess, hint at or foreshadow what comes next — 2 or 3 short plain sentences (at most 55 words) in ${langName(source)}, the essentials only, using the people's names as they appear in the scenes; who = the 2 to 5 people who matter so far, by those names in the VIDEO'S OWN spelling and script (never translated or transliterated); cast = every named person met so far (up to 12), name in that same original spelling, and what they are to the story SO FAR — role: protagonist | antagonist | ally | minor | other; weight: major (drives the story) | minor (passes through); note: at most 12 words in ${langName(source)}, what they are or want, from what was seen so far only.`;
}
